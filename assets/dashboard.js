...
$('#download').on('click', function() {
	...
	html2canvas($('#dashboard')[0], {scale: 2, useCORS: true}).then(function(canvas) {
		...
		pdf.addImage(imgData, 'JPEG', margin, margin, pdfWidth, finalImgHeight);
...
$(document).ready(function() {
	$.getJSON('https://sheets.googleapis.com/v4/spreadsheets/1dI5zeiW_W3w8O6nb13AjrCR51fpnrujmRJyjqY8zBX8/values/Pivots?alt=json&majorDimension=columns&key=AIzaSyBo_nzKxFwcamkPnMMkNPx8ZJrRj852U6Y', function (data) {
		...
		$('path').each(function() {
			...
			var [qtyText,color] = qty>=10000000?[(qty/10000000).toFixed(2)+'Cr','#fff']:qty>=100000?[(qty/100000).toFixed(2)+'L','#fff']:qty>=1000?[(qty/1000).toFixed(2)+'K','#000']:[qty,'#000'];
			var text = document.createElementNS("http://www.w3.org/2000/svg","text");
			$(text).attr({"x":$(this).attr('x'),"y":$(this).attr('y'),"text-anchor":"middle","dominant-baseline":"middle","font-size":"14","font-weight":"600","fill":color,"cursor":"pointer","z-index":"99"});
	...
	$.getJSON("https://api.counterapi.dev/v2/nsgmindia/nsgmindia/up").done(function(response) {const apiCount=response.data?.up_count||0;const hits=1100+apiCount; $('#views').text(hits);}).fail(function() {$('#views').text(1100);});
	$(document).ready(function() {
